...
use crate::utils::{compute_nullifier,build_and_prove_burn_logic,get_account_proof ,compute_remaining_coin,fetch_block_and_header_bytes};
use alloy::{
...
async fn gen_input_witness_proof<P: Provider>(
    ...
    let (block_number_val, header_bytes) = fetch_block_and_header_bytes(provider,block_number).await?;
    ...
    let proof = match (proof, block_number) {
        ...
        (None, None) => {
            get_account_proof(provider,burn_addr).await?
        }
        _ => unreachable!(),
    };
