...
use alloy::{
    ...
    signers::{k256::elliptic_curve::consts::U25, local::PrivateKeySigner},
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    match opt {
        ...
        MinerOpt::Recover(recover_opt) => {
            let net = NETWORKS
                .get(&recover_opt.network)
                .expect("Invalid network!");
            ...
            let wallet_addr = recover_opt.private_key.address();

            let provider = ProviderBuilder::new()
                .wallet(recover_opt.private_key)
                .connect_http(net.rpc.clone());
            ...
            let burn_key = Fp::from_repr(FpRepr(
                U256::from_str_radix(&recover_opt.burn_key, 16)?.to_le_bytes(),
            ))
            .into_option()
